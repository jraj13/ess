...
//"&net_amount=" + net_amount;
 ...
 "&net_amount=" + net_amount+ "&status="+status;
				...
				$.ajax({
					url : 'postLoginJsp/HRController/finalsettelemrnt_step1_insert.jsp',
					type : 'POST',
					data : final_data,
					datatype : 'json',
					success : function(data) {
...
System.out.println("fnfworksheet_hr..."+seq_no);					
...
String service_days="0";
String deduction_days="0";
String month_no="0";
System.out.println("fnf worksheet hr number in full and final hr");
WorkSheet_FandF sheet1=new WorkSheet_FandF();

ArrayList<String> initial_data=sheet1.deciding_months(seq_no);
if (initial_data.size() > 0) {
	for (int j = 0; j < initial_data.size(); j++) {
		String inti_fandf = initial_data.get(j).toString();
		
		String ini_arr[] = inti_fandf.split("\\^");
		
		 service_days=ini_arr[0];
		 deduction_days = ini_arr[1];
		 month_no=ini_arr[2];
		 System.out.println("service_days"+service_days);
		 System.out.println("deduction_days"+deduction_days);
...
int ser_int=Integer.parseInt(service_days);
int ded_int=Integer.parseInt(deduction_days);
int month_number=Integer.parseInt(month_no);

ArrayList<String> data=sheet1.step1_work_sheet(seq_no, ser_int,ded_int,month_number);
if (data.size() > 0) {
