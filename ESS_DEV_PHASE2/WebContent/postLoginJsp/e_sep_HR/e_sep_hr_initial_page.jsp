...
$('.reg_approve').on('click', function(event){
    ...
    var comments_text=document.getElementById("comments_note").value;
    ...
    if (confirm('Are you sure you want to Accept Resignation ?')) {
		...
		var man_accept = "&emp_id=" + emp_id +
							"&creation_date=" + creation_date + "&rileaving_date=" + rileaving_date +"&actual_notice="+actual_notice+"&given_notice="+given_notice+"&emp_note="+emp_note+"&rm_note="+rm_note+"&comments_text=" + 
							comments_text+"&status="+status;
		
		$.ajax({

			url : 'postLoginJsp/e_sep_HR/e_sep_status_decide.jsp',
			type : 'POST',
			data : man_accept,
			datatype : 'json',
...
$('.reg_reject').on('click', function(event){
	var emp_id=($(event.target).closest('tr').find('.emp_id').text());
    ...
    if(given_notice=="")
		...
		status="Reject";
		var man_accept = "&emp_id=" + emp_id +
		"&creation_date=" + creation_date + "&rileaving_date=" + rileaving_date +"&actual_notice="+actual_notice+"&given_notice="+given_notice+"&emp_note="+emp_note+"&rm_note="+rm_note+"&comments_text=" + comments_text+"&status="+status;
...
%> 
 ...
 <td style="background-color: #4682B4">
  <button type="button" id="delete" class="reg_approve" class="glyphicon glyphicon-ok" > 
	...
	<td><input id="cmm_but" type="button" value="Add Comments" onclick="display()"> </td>
</tr> 
 <%
	}
	} else {
...
<tr>
<td >
<textarea  id="comments_note" rows="5" cols="130" placeholder="Enter Comments here"></textarea> 
</td>
</tr>
</table>
	</div>
