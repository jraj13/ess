...
public class Daterange_Expense_Report {
	...
	private static Font bigbold = new Font(Font.FontFamily.TIMES_ROMAN, 12,
		...
		public String report_date_basis() throws IOException{
			
	        HttpServletResponse response = ServletActionContext.getResponse();
	        
	        String report_name="all";
	    ...
	    Daterange_Expense_Report cash_voucher=new Daterange_Expense_Report();
		...
		Chunk sbpart= new Chunk("From Date : " +getFromdate()+"\n ",bigbold);
		Chunk add1= new Chunk("To Date : "+getTodate()+"\n",bigbold);
        ...
        ArrayList<String> data=cash_voucher.bills(type_of_expense,getFromdate(),getTodate());
		  if (data.size() > 0) 
		  {
...
String grant_amount=cash_voucher.calculating_total(type_of_expense,getFromdate(),getTodate());
        ...
        document.add(para_empty);
		  ...
		  else
		  {
			  ...
			  PdfPCell cell;
		        ...
		        ArrayList<String> data=cash_voucher.bills(getType_expense(),getFromdate(),getTodate());
				  if (data.size() > 0) 
				  {
				  	for (int m = 0; m < data.size(); m++) {
				  		String fandf_data = data.get(m).toString();
				  	 ...
				  	 String grant_amount=cash_voucher.calculating_total(getType_expense(),getFromdate(),getTodate());
				  	 cell=new PdfPCell(new Phrase("Total Amount",bigbold));
				        cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				        cell.setColspan(6);
				        tot_para.addCell(cell);
	    ...
	    document.close();
	   	...
	   	response.addHeader("Content-Disposition", "attachment; filename=\" "+report_name  +" Report.pdf\"");
	...
	public ArrayList<String>  bills(String type_expense_entry,String month_in,String year_in) throws SQLException, ParseException
	{
		...
		String from_date_str=month_in;
		String to_date_str=year_in;
	...
	/*String expense_sql=	"select MONTH_IN ,PERIOD ,BILL_NO ,BILL_DATE ,DUE_DATE ,AMOUNT ,REMARKS , IN_DATE   "
		...
		String expense_sql="select MONTH_IN ,PERIOD ,BILL_NO ,BILL_DATE ,DUE_DATE ,AMOUNT ,REMARKS , IN_DATE  from "
			+ "MONTHLY_EXPENSES where TYPE_EXPENSE='"+type_expense_entry+"' and IN_DATE >= '"+from_date+"' and IN_DATE <='"+to_date+"'";
...
String expense_amt="select sum(AMOUNT) from MONTHLY_EXPENSES where TYPE_EXPENSE='"+type_expense_entry+"' and IN_DATE >= '"+from_date+"' and IN_DATE <='"+to_date+"'";
